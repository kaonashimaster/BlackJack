...
# --- ここから処理開始 ---
def main():
    parser = argparse.ArgumentParser(description='OpenAI Gym CartPole-v1 Q-Learning Discount Factor Experiment')
    parser.add_argument('--games', type=int, default=5000, help='num. of games to play for each DF')
    ...
    # --- 実験するDISCOUNT_FACTORの値のリスト ---
    df_values = [0.1, 0.3, 0.5, 0.7, 0.9]
    results = {} 

    env = gym.make(GAME_NAME)

    # --- 各DISCOUNT_FACTOR値で学習を実行 ---
    for df_value in df_values:
        print(f"\n--- Starting training for DISCOUNT_FACTOR = {df_value} ---")
        ...
        results[df_value] = rewards_per_game
        print(f"--- Finished training for DISCOUNT_FACTOR = {df_value} ---")
    ...
    for df_value, rewards in results.items():
        avg_rewards = moving_average(rewards, window_size)
        games_for_avg = np.arange(window_size, N_GAMES + 1)
        plt.plot(games_for_avg, avg_rewards, label=f'DF = {df_value}')

    plt.xlabel("Games")
    plt.ylabel(f"Average Reward (Moving Average over {window_size} games)")
    plt.title(f"CartPole Q-Learning Performance (EPS={EPS}, LR={LEARNING_RATE})")
    plt.legend()
    plt.grid(True)
    plt.savefig("cartpole_df_comparison.png")
    print("\nGraph saved as cartpole_df_comparison.png")
    plt.show()
