...
def act(action):
    ...
    # RETRYの場合はコスト支払いが発生する (★修正点1)
    if action == Action.RETRY:
        # ベットの1/4を支払う
        retry_cost = int(BET / 4)
        player.money -= retry_cost
        
        # 学習用の報酬としても、コスト分（勝利報酬1.0に対しての比率）をペナルティとして与える
        # 勝率を上げるためには「無駄なRetry」を避けるべきと学習させる
    ...
    # ステータスごとの報酬設定 (★修正点2: バースト時の処理確認)
    if status == 'win' or status == 'dealer_bust':
        learning_reward += 1.0
    elif status == 'lose':
        learning_reward += -1.0
    elif status == 'bust':
        # バーストしたら即負け。RETRY後でもHIT後でも同様に大きなペナルティ
        learning_reward += -5.0 
    ...
    # バースト時は必ず done=True になるはずだが、念のため status チェック
    if status == 'bust':
        done = True
...
def main():
    ...
    for game_ID in range(1, total_games + 1):
        ...
        # ゲーム開始前の所持金を記録
        money_before_game = player.get_money()
        ...
        while True: 
            ...
            if done:
                current_game_status = status # 最終ステータスを保存
                ...
                if status == 'win' or status == 'dealer_bust':
                    ...
                    # lose, bust, surrendered は負け扱いとしてカウント
                    total_loses += 1
                break
        ...
        profit = current_money - money_before_game # 今回の勝ち額（負けならマイナス）
        current_win_rate = (total_wins / game_ID) * 100
    ...
    print(f"Results Breakdown: Wins: {total_wins}, Loses: {total_loses}, Draws: {total_draws}")
    
    # 全ゲームのログ表示
    print("\n" + "="*70)
    print(f"{'Game':^6} | {'Result':^12} | {'Money':^10} | {'Profit':^10} | {'Win Rate':^10}")
    print("-" * 70)
    for log in all_game_logs:
        print(f"{log['game_id']:6d} | {log['status']:<12} | {log['money']:10.1f} | {log['profit']:+10.1f} | {log['win_rate']:9.2f}%")
    print("="*70 + "\n")
