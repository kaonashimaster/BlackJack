...
EPS_START = 1.0
EPS_END = 0.1
EPS_DECAY_STEPS = 20000 # 2万ゲームかけて探索を減らす
EPS = EPS_START

# ★修正: 学習率を少し上げて、割引率を少し下げる（ai_yossi寄りかつ安定重視）
LEARNING_RATE = 0.001 
...
# ★追加: ターゲットネットワークの更新頻度
TARGET_UPDATE_FREQ = 500  # 500学習ステップごとに同期
...
try:
    ...
    target_net.eval() # ターゲットは学習しないので推論モード固定
...
def train_step():
    ...
    # ★修正: 次のQ値はターゲットネットワークで計算
    with torch.no_grad():
        # Double DQNにするならここで「行動選択はq_net、価値はtarget_net」にするが、
        # まずは標準的なDQN(Target Networkあり)で実装
...
### メイン処理 ###
def main():
    global g_retry_counter, EPS, target_net, q_net
    ...
    for n in range(1, n_games):
        ...
        while True:
            ...
            # === 報酬の正規化 (Reward Shaping) ===
            # 金額ベースの正規化 (+1.0 / -1.0)
            normalized_reward = reward / player.basic_bet
            
            # 特殊ケースの調整
            if action == Action.RETRY:
                # RETRYは負け(-1.0)と同等のペナルティを与える
            ...
            if not args.testmode:
                ...
                if loss is not None: 
                    ...
                    if total_steps % TARGET_UPDATE_FREQ == 0:
                        target_net.load_state_dict(q_net.state_dict())
                        # print(f"Target Net Updated: {total_steps}")
